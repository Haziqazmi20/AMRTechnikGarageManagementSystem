<?php include('C:\xampp\htdocs\AMRTechnik\config\constant.php');
...
if (isset($_POST['submit'])) {
    ...
    $select_users = mysqli_query($conn, "SELECT * FROM `mechanic` WHERE id = '$id' AND username = '$username' AND password = '$pass'") or die('query failed');

    if (mysqli_num_rows($select_users) > 0) {
        $row = mysqli_fetch_assoc($select_users);

        if ($row['username'] == $username) {
            $_SESSION['mechanic'] = $row['username'];
